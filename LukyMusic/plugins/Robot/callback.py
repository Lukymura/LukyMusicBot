...
from LukyMusic import YouTube, app
from LukyMusic.core.call import Luky
from LukyMusic.misc import SUDOERS, db
from LukyMusic.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from LukyMusic.utils.decorators.language import languageCB
from LukyMusic.utils.formatters import seconds_to_min
from LukyMusic.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from LukyMusic.utils.stream.autoclear import auto_clean
from LukyMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Luky.pause_stream(chat_id)
        ...
        await Luky.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Luky.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Luky.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Luky.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Luky.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
